...
def make_all_combos():
    ...
    with tqdm(total=17721088, desc='Make_all_combos', leave=True, ncols=100, unit='B', unit_scale=True) as pbar:
        # 将combos 打印到文件中
...
def make_red_combos():
    ...
    with tqdm(total=17721088, desc='Make_red_combos', leave=True, ncols=100, unit='B', unit_scale=True) as pbar:
        # 将combos 打印到文件中
        with open(path_combos, 'w') as f:
            for c in result:
                f.write(c + '\n')
